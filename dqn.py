...
def optimize(dqn, target_dqn, memory, optimizer):
    ...
    predictions = dqn.forward(observations)
    if dqn.env_name == 'Pong-v0':
        actions = torch.tensor(list(map(f, actions)))
    q_values = torch.gather(predictions, dim=1, index=actions.unsqueeze(dim=1))

    # TODO: Compute the Q-value targets. Only do this for non-terminal transitions!

    target_values = target_dqn.forward(next_observations)  # target q-values for next_states
